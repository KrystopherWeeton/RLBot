...
import rlbot.utils.structures.ball_prediction_struct as rl_ball_prediction
...
from util.util import get_angle, get_post_collision_velocity
...
class Shoot(State):
    ...
    CONTACT_Z_THRESH: float = 150.0
    contactSlice: rl_ball_prediction.Slice = None
    DIST_THRESHOLD: float = 400
    POWERSLIDE_THRESHOLD: float = 1
    STOP_AND_WAIT_RADIUS: float = 600
    

    def get_time(self, time, slice: Slice):
        return slice.game_seconds - time
    
    def score(self, parsed_packet: ParsedPacket, packet: rl.GameTickPacket, agent: BaseAgent) -> float:
        return None
    ...
    def chooseContactPoint(self, slices: [Slice], carPhysics, agent: DrawingAgent):
        ...
        for i, cand in enumerate(candidates):
            score, logData = self.score_candidate(init_time, cand, carPhysics, agent)
            if (score > optimalScore):
                optimalSlice = cand
                optimalScore = score
                optimalDelta = logData["delta"]
            
        if(optimalSlice != None):
            self.contactSlice = optimalSlice
            agent.draw_rects([optimalSlice.physics.location], agent.renderer.blue())
            agent.write_string(optimalSlice.physics.location, f"{optimalDelta:.2f}", 1, agent.renderer.lime())
        else:
            self.contactSlice = None
